        ...
        while(choice != 0) { // Continue the loop until the user selects 0
            ...
            switch(choice) {
                ...
                case 2: { // Service removal option
                    ...
                    while(my_service.get_service_state(SERVICE_NAME) != 1) {
                        std::cout << "Service is being stopped" << "\n";
                        std::this_thread::sleep_for(std::chrono::seconds(1)); // Wait for 10 seconds
                        if(my_service.get_service_state(SERVICE_NAME) == -1) {
                            break;
                        }
                    }
                ...
                default: // Invalid option
                    std::cerr << "Usage: " << choice << " [0|1|2]" << std::endl; // Error message
                    break;
            }
        }
    }
